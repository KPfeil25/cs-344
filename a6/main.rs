...
use std::thread; // needed to create threads
...
fn main() {
    ...
    let xs0_clone = xs[0].clone(); // clone the element
    let xs1_clone = xs[1].clone();

    let th1 = thread::spawn(move || map_data(&xs0_clone)); // spawn the threads, return the handles
    let th2 = thread::spawn(move || map_data(&xs1_clone));
    let join1 = th1.join().unwrap(); // get the output from the threads
    let join2 = th2.join().unwrap();

    intermediate_sums.push(join1); // push the results
    ...
    let xs2 = partition_data(num_partitions, &v); // create this in order to partittion the data since xs isnt mutable
    print_partition_info(&xs2); // print the info

    let mut thread_vec = vec![]; // used to hold all of the threads
    let mut intermediate_sums2 : Vec<usize> = Vec::new(); // storing the intermediate results

    for i in 0..num_partitions { // loop through the partitions
        let new_clone = xs2[i].clone();
        let new_t = thread::spawn(move || map_data(&new_clone)); // spawn the thread
        thread_vec.push(new_t); // push it into the vector of thread
    }
    for thread in thread_vec { // for each loop for the threads in the thread vector
        intermediate_sums2.push(thread.join().unwrap()); // push the results after joining the threads
    }
    println!("Intermediate sums = {:?}", intermediate_sums2); // print intermediate sums
    let sum = reduce_data(&intermediate_sums2); // reduce the data
    println!("Sum = {}", sum); // print again
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
    ...
    let mut remainder = v.len() % num_partitions; // get the number of odd partitions
    let mut isuckatcoding = remainder; // need these for the second while loop
    ...
    // Add the first half of the integers in the input vector to x1
    let mut k = 0;
    while remainder > 0 { // handles the odd numbered partitions
        x1 = Vec::new();
        for j in 0..(partition_size + 1) { // run through +1 in order to add the remaining elements
            x1.push(v[k]); // push the elements
            k += 1; // increment k
        }
        remainder -= 1; // decrement the remainder to signify that we have handled an odd partition
        //println!("x1 = {:?}", x1);
        //println!("remainder = {}", remainder);
        xs.push(x1); // push the vector
    }
    while num_partitions - isuckatcoding > 0 { // same as above, just handling the rest of the partitions
        x1 = Vec::new();
